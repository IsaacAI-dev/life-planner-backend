...
async function main() {
  ...
  const siteContent = {
      ...
      supportHours: 'Mon–Fri, 09:00–18:00 WAT',
  ...
  // --- About Us: public staff profiles on Admin rows -------------------------
  // The seed admins represent the team for demo purposes. We mark the named
  // team members as publicly visible with curated staff titles, using upsert
  // so reseed doesn't overwrite any edits already made in the console.
  const staffProfiles = [
    { email: 'admin@lifeplanner.local',   staffRole: 'Founder & CEO',       staffSortOrder: 0, bio: 'Built Life Planner after years of watching good plans die in bad tools.',       isPublic: true },
    { email: 'coach@lifeplanner.local',   staffRole: 'Head of Coaching',    staffSortOrder: 1, bio: 'Ten years in behaviour change; leads the coaching desk.',                       isPublic: true },
    { email: 'manager@lifeplanner.local', staffRole: 'Head of Engineering',  staffSortOrder: 2, bio: null,                                                                           isPublic: true },
    { email: 'support@lifeplanner.local', staffRole: 'Nutrition Lead',      staffSortOrder: 3, bio: null,                                                                           isPublic: false },
  ];
  for (const { email, ...profile } of staffProfiles) {
    await prisma.admin.updateMany({ where: { email }, data: profile });
  }

  // --- Marketing site --------------------------------------------------------
  // Image slots are seeded empty on purpose. A row with a null `imageUrl` is
  // how the site says "this slot is known and still unfilled", which renders
  // the designed placeholder; inventing stock URLs here would put broken
  // images on the landing page the first time anyone runs this.
  const MARKETING_SLOTS: { slot: string; key: string; label?: string; sortOrder: number }[] = [
    { slot: 'HERO_PREVIEW', key: 'hero', sortOrder: 0 },
    { slot: 'BEND_PRIMARY', key: 'primary', sortOrder: 0 },
    { slot: 'BEND_DETAIL', key: 'detail', sortOrder: 0 },
    { slot: 'ABOUT_HERO', key: 'about', sortOrder: 0 },
    ...['today', 'calendar', 'goals', 'boards', 'chat'].map((key, i) => ({
      slot: 'SCREEN',
      key,
      sortOrder: i,
    })),
    // Names are matched verbatim by the frontend — see docs/MARKETING_ENDPOINTS_DELIVERED.md.
    ...[
      'Priya Raman',
      'Daniel Okoro',
      'Elin Sandberg',
      ...
      slot: 'TESTIMONIAL_PORTRAIT',
      key: label.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
      label,
      sortOrder: i,
    })),
    ...['Ada Kwan', 'Marcus Oyelaran', 'Rina Halvorsen', 'Tomás Ferreira'].map((label, i) => ({
      slot: 'TEAM_PORTRAIT',
      key: label
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
  ...
  for (const slot of MARKETING_SLOTS) {
    await prisma.marketingAsset.upsert({
      where: { slot_key: { slot: slot.slot as never, key: slot.key } },
      // Never overwrites a real image with null on reseed.
      update: { label: slot.label ?? null, sortOrder: slot.sortOrder },
      create: {
        slot: slot.slot as never,
        key: slot.key,
  ...
  if ((await prisma.marketingFaq.count()) === 0) {
    await prisma.marketingFaq.createMany({
      data: [
        {
          question: 'Is the free plan actually free, or is it a trial?',
          answer:
            'Actually free, with no time limit and no card required. The planner, goals and Support chat stay free; coaching, voice notes and meal plans are Plus.',
          sortOrder: 0,
        },
        {
          question: 'Can I pay for someone else?',
          answer:
            'Yes — a plan can cover two or three people. Everyone keeps their own private account; you are paying for their access, nothing more.',
          sortOrder: 1,
        },
        {
          question: 'Can my partner see my planner?',
          answer:
            'Only if you share a board with them, and only what you choose to share. Nothing is shared by default.',
          sortOrder: 2,
        },
        {
          question: 'What happens if I cancel?',
          answer:
            'You keep Plus until the end of the period you have paid for, then drop to Free. Nothing is deleted.',
          sortOrder: 3,
        },
        {
          question: 'Is there an app?',
          answer:
            'The web app works on a phone today. Native iOS and Android apps are in progress — the store badges appear here when they land.',
          sortOrder: 4,
        },
        {
          question: 'Who are the coaches?',
          answer:
            'Real people on our team, not a chatbot. You are matched with one, and you keep them.',
          sortOrder: 5,
        },
        {
          question: 'Where is my data stored, and can I get it out?',
          answer:
            'On our servers in the EU, and yes — you can export everything you have written, at any time, from Settings.',
          sortOrder: 6,
        },
      ],
    });
  ...
  // Two open roles rather than none: the empty state matters too, so delete
  // these to see it — the careers page is built for both.
  if ((await prisma.careerRole.count()) === 0) {
      ...
      data: [
        {
          ...
          body: 'TypeScript and Postgres, end to end. You will own the calendar and flexible-task engine with one other engineer.',
        ...
        {
          title: 'Coach (life & habits)',
          slug: 'coach-life-habits',
          department: 'Coaching',
          body: 'Twenty to thirty clients, in chat, with real time to think between replies. Behaviour-change background matters more than a coaching certificate.',
          location: 'Remote',
  ...
  // The sign-up consent line. A lead-in fragment, not a sentence: the frontend
  // appends its own Terms and Privacy links after it.
  ...
  // No AppStoreLink rows on purpose: neither native app exists, so the badge
  // row is hidden, which is the truthful default.

  // --- /terms and /privacy legal documents -----------------------------------
  // Seeded verbatim so both pages are live from first boot without a console
  // visit. Editing them creates a new version, never overwrites the old one —
  // so the history of what any given user agreed to is always recoverable.
  const legalDocs = [
    {
      slug: 'terms',
      version: '2026-08-02',
      title: 'Terms & Conditions',
      intro: 'These terms govern your use of Life Planner. By creating an account you agree to them.',
      publishedAt: new Date('2026-08-02T00:00:00.000Z'),
      sections: [
        { h: 'Your account', p: 'You are responsible for keeping your login details secure. Do not share your password. If you suspect unauthorised access, change your password immediately and contact support.' },
        { h: 'Acceptable use', p: 'Use Life Planner only for lawful purposes. Do not attempt to reverse-engineer, overload, or disrupt the service. Do not upload content that violates the law or harms others.' },
        { h: 'Your content', p: 'Everything you write in Life Planner — tasks, notes, goals — belongs to you. We do not read it, sell it, or train AI models on it. You can export or delete it at any time from Settings.' },
        { h: 'Our service', p: 'We aim for high availability but cannot guarantee uninterrupted access. We may update or discontinue features with reasonable notice. We will never delete your account without telling you first.' },
        { h: 'Payments and refunds', p: 'Subscriptions renew automatically. You can cancel at any time from Settings; you keep access until the end of the paid period. We offer refunds at our discretion — contact support within 14 days of a charge if you believe one was in error.' },
        { h: 'Limitation of liability', p: 'Life Planner is provided as-is. To the fullest extent permitted by law, we are not liable for indirect or consequential losses arising from your use of the service.' },
        { h: 'Changes to these terms', p: 'We will notify you by email at least 14 days before material changes take effect. Your continued use after that date constitutes acceptance.' },
        { h: 'Governing law', p: 'These terms are governed by the laws of England and Wales. Any dispute will be subject to the exclusive jurisdiction of the English courts.' },
        { h: 'Contact', p: 'For questions about these terms, email legal@lifeplanner.co.' },
      ],
    },
    {
      slug: 'privacy',
      ...
      intro: 'This policy explains what data we collect, why we collect it, and the choices you have.',
      publishedAt: new Date('2026-08-02T00:00:00.000Z'),
      sections: [
        { h: 'What we collect', p: 'We collect the information you give us when you sign up (name, email, password hash) and the content you create in the app (tasks, goals, notes, meal plans, budget entries). We also collect basic usage logs (IP address, browser, timestamps) for security and performance.' },
        { h: 'What we do not collect', p: 'We do not read the content of your planner entries. We do not sell your data. We do not run advertising on Life Planner.' },
        { h: 'How we use your data', p: 'We use your account data to provide and improve the service, to send you notifications you have requested, and to process payments. We use usage logs to detect abuse and diagnose problems.' },
        { h: 'Coaching and support', p: 'If you use the coaching or support chat, your messages are seen by the coach or support agent assigned to you. They are bound by confidentiality obligations. Conversations are stored for continuity and quality.' },
        { h: 'Data sharing', p: 'We share data with payment processors (Paddle, Paystack, Apple, Google) only as needed to process transactions. We share data with infrastructure providers (hosting, email delivery) under strict data-processing agreements. We do not share your data with any other third parties.' },
        { h: 'Data retention', p: 'Active account data is kept for as long as your account exists. If you delete your account, we delete your personal data within 30 days, except where we are required by law to retain it.' },
        { h: 'Your rights', p: 'You can export all your data at any time from Settings. You can delete your account from Settings. You can request a copy of your data or ask us to correct inaccuracies by emailing privacy@lifeplanner.co.' },
        { h: 'Cookies', p: 'We use a single session cookie to keep you logged in. We do not use third-party tracking cookies or analytics cookies.' },
        { h: 'Security', p: 'Passwords are hashed with Argon2id. All data is encrypted in transit (TLS) and at rest. We perform regular security reviews.' },
        { h: 'Changes to this policy', p: 'We will notify you by email at least 14 days before material changes take effect.' },
        { h: 'Contact', p: 'For privacy questions or data requests, email privacy@lifeplanner.co.' },
      ],
    },
  ];
