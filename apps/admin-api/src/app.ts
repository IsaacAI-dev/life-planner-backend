...
import {
  ...
  legalConsentRouter,
  legalDocumentsRouter,
  marketingAssetsRouter,
  marketingFaqsRouter,
} from './modules/marketing.js';
...
export const createApp = () => {
  ...
  // About Us staff profiles — backed by Admin rows, not a separate table.
  api.use('/about-us', requireAdmin, aboutUsRouter);
  ...
  // --- Marketing site ---
  // Reads open to any admin, writes oversight-only (enforced per route), except
  // the contact inbox: triaging it is support work.
  api.use('/marketing-assets', requireAdmin, marketingAssetsRouter);
  api.use('/faqs', requireAdmin, marketingFaqsRouter);
  api.use('/careers/roles', requireAdmin, careerRolesRouter);
  api.use('/app-links', requireAdmin, appLinksRouter);
  api.use('/legal/consent', requireAdmin, legalConsentRouter);
  api.use('/legal/documents', requireAdmin, legalDocumentsRouter);
  api.use('/contact-submissions', requireAdmin, contactSubmissionsRouter);
